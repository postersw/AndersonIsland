...
//                           unixtimestamp,date,A/S,ONTIME/LATE/CANCELLED,delaytime in min, next run time
...
//  1.54 3/3/24.   Calculates Cancelled runs using an array of runs.
...
// ferry run time arrays.  Loaded/saved in $SAVED.  Initialized daily by BuildFerrySchedule
...
function checkforLateFerry2() {
    ...
    switch($ferrystate) {
        ...
        case "atAI": // docked at AI
            if($priorferrystate != "atAI") {  // if ferry was coming to AI, it has just arrived, so save its arrival time
                ...
                $SAVED["ferryarrivaltimeMMAI"] = $ferryarrivaltime; //  file_put_contents("ferryarrivaltimeMMAI", $nowMM);
    ...
    $delaymsg = "<span style='color:white;background-color:red'>&nbsp;Late&nbsp;</span><span style='color:red'>&nbsp;$delaytime m for $ferryport " . ftimeMM($nextrunMM)  . " run. $dETD</span>";  
...
// function gettimeofnextrunMM($STAI)  {
...
function LogFerryRun($SA, $ont = "", $waitingforrunMM="") {
    ...
    if($i===false) {
        echo "ERROR LogFerryRun: cant find run time $runMM $ont for $SA in gFerryTimes)";
        return;
    }
...
function CheckForCancelledRuns($waitingforMM) {
    ...
    for($i=0; $i<count($FerryTimes); $i++) {
        if(($wfhhmm >= $gFerryTimes[$i]) && ($gFerryStatus[$i]=="")) {
            // Run has been skipped. Mark it and log it.
            LogFerryRun($gFerryLoc[$i], "CANCELLED", $gFerryTimes[$i]);
            echo "Ferry run CANCELLED {$gFerryLoc[$i]}, {$gFerryTimes[$i]}, waiting for $waitingfor.<br> ";
        }
    }
}   
...
function testgetnextrun() {
    ...
    for($i=0; $i<(24*6); $i++) {
        ...
        echo "$hhmm ST=" . ftimeMM($nrST) . ", AI=" . ftimeMM($nrAI) . "<br><br>";
    }
}
